...
Input:
["BrowserHistory","visit","visit","visit","back","back","forward","visit","forward",
"back","back"]
...
class BrowserHistory:
    ...
    def back(self, steps: int) -> str:
        ...
        self.position = new_position if new_position >= 0 else 0
        # print(f'{self.position} {self.history}')
        return self.history[self.position]

    def forward(self, steps: int) -> str:
        new_position = self.position + steps
        l = len(self.history) - 1
        self.position = new_position if new_position <= l else l
        return self.history[self.position]
